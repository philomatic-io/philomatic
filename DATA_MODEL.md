...
> - **History: the git log.** How the model got here is in the commit history, in build order.
>
> **Change protocol.** The model is expected to take *slight* adjustments from feedback — it is
> stable, not sacred. Any change here crosses the lock line (see CONTRIBUTING.md): deliberate,
> rare, and made once. The rule: **a PR that edits `src/schema/` must update this document in
> the same PR.** Changes must preserve the load-bearing invariants — above all: content-derived
> ids, append-only observations, explicit membership.
...
|---|---|---|
| **Entity** | a node with a deterministic id and fields | idempotent **MERGE-PATCH** upsert (kind-3, 2026-08-09 — see below): per field, absent = keep, explicit null = clear, present = replace |
...
**Merge-patch entity import (kind-3 model decision, 2026-08-09 — the model-stability tripwire taken at two
incidents, deliberately early).** The former rule was full-replace (every column set from the
incoming row), so a partial payload silently wiped fields it never mentioned — twice in
production (adapters nulling `estimatedDurationMins`; propose skeletons wiping
author/duration/tags/modality), with per-writer guards accreting. Now the ONE write gate
merges, RFC-7386 style, per field against the prior row: **absent → keep what the store has;
explicit `null` → clear (optional scalars only); present value → replace (a present `[]`
clears tags/aliases — empty is expressible, absence is not emptiness).** Create-defaults
(`status: active`, `modality: other`, `locked: false`, `validationState: PENDING`, empty
arrays) apply only to a brand-new row. Identity fields (id, name, text, `directUrl`,
`snippet.sourceId`) are never clearable — the `update` primitive's rule, now shared. The
shapes: `Canonical*PatchSchema` / `CanonicalPatchPayloadSchema` (the import side, what
`desugar` emits — absence preserved end to end); the FULL `Canonical*Schema` remain the value
shape reads and exports always see, and a full payload is a valid patch, so backup/fork/publish
round-trips are unchanged. **The priced trade:** "same payload → same result" now holds only
for full-entity payloads (every export); a *partial* payload's result deliberately depends on
store state — that is the point. `track.published`/`origin` keep their pre-existing sentinel
carry-forward (the original hand-rolled instance of this rule). The future assertion layer
supersedes all of this with per-asserter folds; merge-patch is the single-writer-era floor.
...
| Learner | opaque (`lnr_default` is the seeded single tenant). **Never bake a learner id into an id derivation or cross-reference** — the multi-tenant re-key (`rekey-learner`, a pure map over learners/overlay edges/events/creatorId) must stay pure |
...
- **Learner** — `{id, displayName, profile?}`. The tenant whose behavior overlays the shared
  ...
  multi-tenant structures it (Phase 2).
...
- **Track** — `{id, creatorId, title, goal?, framework?, locked, validationState, tags[]}`.
  ...
  the membership invariant. `framework` names an opted-in rigid framework
  (Phase 2); `locked` selects static vs. dynamic upkeep.

**Sentiment is a free string** (in `ANNOTATES` metadata / `#sentiment:*` tags), not an enum —
the engine stays agnostic to UX vocabulary.
...
| `STAGED` | learner→source/concept/question/snippet | Overlay: **pending validation** — proposed or parked |
| `CONSUMED` | learner→source | Overlay: read state (source-only) |
...
validation reports, never import failures).
...
Claim/Answer nodes; edge reification arrives with assertion ids). A ternary edge
...
flowchart LR
  ...
  learner -- STAGED --> concept
  learner -- STAGED --> question
  learner -- STAGED --> snippet
  learner -- STAGED --> track
...
*vocabularies* (rendering only) — in F0 **on learner-overlay edges only** (shared-edge
...
front ends amplify the edge-metadata clobber (§8) — clients must read-modify-write through
...
first genuinely now-dependent read (decay) is deferred.

**Staged is pending validation, and its exits say why** (capture-first D2/D6, 2026-07-25).
`STAGED` marks any entity as *proposed* (by an adapter/LLM) or *parked* (by hand) — awaiting the
learner's verdict. It leaves that state three ways, and because an un- verb may only ever REVERSE
its verb, the two VERDICTS get their own words:

| Exit | Meaning | Effect |
|---|---|---|
| `ACCEPTED` | verdict — the proposal becomes an ordinary entity | STAGED edge removed; verdict logged |
| `REJECTED` | verdict — the proposal is refused | STAGED edge removed; a `RETRACTED` event rides along (append-only, restorable); verdict logged |
| `UNSTAGED` | no verdict — plain reversal ("oops"/un-park) | STAGED edge removed |

All three are event-only. Together with `STAGED` they form the **candidate-disposition log**: the
provenance answer to "why is this in my graph", and labeled data for future ranking (rejections
are signal — never physical deletions). Like `CONSUMED`⇄`UNCONSUMED`, desugar folds the family
latest-wins, so re-importing a payload can never resurrect a settled marker.
...
`RETRACTED` — a pure fold over **exactly that pair** (`RETRACTION_VERBS`; `liveView` in
`src/engine/read.ts`). It deliberately ignores the other event-only verbs: folding all of them
latest-wins meant a later `UNCONSUMED`/`ACCEPTED` read as "latest, not `RETRACTED`" and silently
un-removed the entity (fixed with the staged lifecycle, 2026-07-25). Human-facing projections
...
behavioral overlay) are deferred until assertions give statements addressable ids (Phase 2).
...
five load-bearing invariants bind every model change. Retraction-as-observation for `remove` is
now **as built** (§6); the remaining reserved evolution path (per-asserter **assertions** with
deterministic ids) is designed for Phase 2.
...
- **Edge metadata is the first migration target of the assertion layer**. It is the
...
- **The retraction fold is sync's most clock-skew-sensitive consumer**: latest-wins
  by `occurredAt` means skewed devices can resurrect deletions. Phase-2 clock-skew handling
  must be designed against this fold first, not average-case event ordering.
- **The learner re-key must stay a pure map**: see §2's learner row.
...
- **Deferred fields** (designed, never populated in Phase 1): `embedding` (source: recommendation
  ...
  unit of annotation meaning). Phase 2.
- **Domain machinery: removed** (rode the retraction kind-3 change, 2026-07). `EntityKind` no
  ...
  Domain entity ever existed (the reverted Domain-as-entity decision).
  Domains are `#domain:*` tag observations + derived views; hierarchy, if a consumer ever needs
  it, is the deferred `BROADER` concept→concept edge.
