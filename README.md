...
**[open the workbench →](https://philomatic.io/app)** and choose *keep it in this browser* —
the full engine compiled to WebAssembly, running and persisting entirely in your browser;
nothing you do there leaves your machine, and no install is required. The `examples/` folder
has sample graphs to import if you want something to explore before capturing your own.
...
shared — and anyone can run one: `philomatic registry`.
...
```bash
pnpm install
npx tsc --noEmit -p .          # engine + servers typecheck
npx tsc --noEmit -p ui         # workbench typecheck
pnpm test                      # the root suite
pnpm ui:build && npx vitest run test/ui-smoke   # deploy-shape smokes (need a chromium)
npx eslint .                   # react-hooks rules + max-lines
pnpm comments:report           # comment style of record — must be zero
```

Two rules a first PR must know, both in `CONTRIBUTING.md`:

- **The lock line.** No module is shared across `src/` (node engine + servers) and `ui/` (the
  workbench). Where the two sides must agree on a wire shape, each keeps its own copy annotated
  `Deliberate twin: <path>`, and `test/drift-twins.test.ts` pins the pairs. Extract shared
  logic *within* a side, never across the line.
- **Engine parity.** The two clients — HTTP transport and the in-browser engine — must not
  drift. Behavior lives in the engine, not in a client; land contract-test cases in the same
  change. `CLAUDE.md` has the fuller orientation for automated agents and humans alike.

## Where things are decided

- `DATA_MODEL.md` — the living reference for entities, edges, events, and the id rules.
- `CONTRIBUTING.md` — code and comment standards, the lock line, the parity discipline.
- `docs/OPERATIONS.md` — the deploy shape, routing, and environment variables for self-hosters.
