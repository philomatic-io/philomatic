/**
 * The UI smoke harness (maintainability phase 4a, 2026-07-23) — boots a REAL workbench over a
...
export async function startWorkbench(): Promise<Workbench> {
  ...
  const pathCol = () => page.locator('.journey-col').nth(1);
  ...
  return {
    ...
    markers: async () => await pathCol().locator('.col-row .path-num').allInnerTexts(),
    rowTitles: async () => await pathCol().locator('.col-row .row-title-text').allInnerTexts(),
    undo: async () => {
    ...
    dragTo: async (fromSel, fromIdx, toSel, toIdx, frac) => {
      // eslint-disable-next-line @typescript-eslint/no-unsafe-argument -- runs in the browser realm
