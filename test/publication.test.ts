...
// ── the static export; fork import with lineage ────────────────────────────────────────────────
...
describe('fork import with lineage (P4)', () => {
  ...
  it('records origin once, verifies the contentHash, and archives beside a file-backed DB', () => {
    ...
    expect(sy.origin).toEqual({
      ...
      authorKey: (bundle.publication as { authorKey?: string }).authorKey, // the TOFU pin at fork
    });
...
// ── author keys — signed manifests, verify + TOFU on fork ──────────────────────────────────────
