/**
 * Snippet text edit-by-supersession (the raw-source toggle makes
...
describe('update({text}) on a snippet', () => {
  ...
  it('a formatting-only edit (same normalized id) updates in place — no self-collision', () => {
    ...
    // SAME id. The supersession path must not refuse the snippet against itself —
    // a formatting-only edit is an in-place text update instead.
    const engine = build();
    const oldId = engine.snapshot().snippets.find((s) => s.text === 'Old passage.')!.id;
