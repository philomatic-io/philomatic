/**
 * Text resolvers (one family, shared with acquisition):
 * deterministic write-time normalization, BEFORE the engine derives identity (text
 * participates in the snippet id). First resolver: MediaWiki's math paste artifact —
 * {\displaystyle x^{2}} → $x^{2}$.
 */
import { describe, expect, it } from 'vitest';
import { normalizeText } from '../src/server/adapters';
import { createIngestServer } from '../src/server/ingest';
import type { Server } from 'node:http';
import type { AddressInfo } from 'node:net';

const N = (t: string) => normalizeText('https://en.wikipedia.org/wiki/Normality_test', t);
...
describe('tex-paste-artifacts resolver', () => {
  ...
  it('leaves math-free text byte-identical and is failure-isolated', () => {
    ...
    expect(normalizeText('u', 'text', boom)).toBe('text');
  });
});
