/**
 * Edit transport + CLI, thin by construction:
...
describe('edit M3: HTTP transport', () => {
  ...
  it('guards the edit writes with the ingest token', async () => {
    ...
    // Reads are guarded too: a token-protected server must not hand its whole library
    // to any GET — that is not what setting a token means.
    expect((await fetch(`${base}/removed`)).status).toBe(401);
    expect((await fetch(`${base}/removed`, { headers: { 'X-Ingest-Token': 'sesame' } })).status).toBe(200);
    // …but what is deliberately public stays public, or publishing and ask links break.
    expect((await fetch(`${base}/health`)).status).toBe(200);
    expect((await fetch(`${base}/framework`)).status).toBe(200);
