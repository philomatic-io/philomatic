/**
 * Read-contract additions + the assemble route:
 *   - SourceView.about: concept names via EXPLAINS (the concept facet for sources)
 *   - SnippetView.raises: the snippet's questions with asked/answered/gap overlay
...
describe('alpha M1: read-contract additions', () => {
  ...
  it('TrackView.sourceLevels layers members by in-context PRECEDES; co-requisites share a level', () => {
    ...
    expect(syl.precedes).toEqual([{ srcId: idA, dstId: idB, trackContextId: syl.id }]);
