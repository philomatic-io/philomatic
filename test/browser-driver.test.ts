 ...
 * verbs. sql.js reports no rows-affected, so a `changes` check in deleteEdge would silently
 * skip the UNCONSUMED event and the surviving CONSUMED event would resurrect the edge on
 * export→import replay (the browser backend's persistence cycle).
 */
...
describe('browser driver (sql.js)', () => {
  it('unconsume reports changed, records the counter-event, and survives export→import replay', async () => {
    ...
    // The browser backend's persistence cycle: exportAll → fresh engine → importPayload.
