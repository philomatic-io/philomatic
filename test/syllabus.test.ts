...
describe('Slice 5: track', () => {
  ...
  it('desugars a track into INCLUDES + scoped PRECEDES edges', () => {
    ...
    const sid = trackId('S');
    // Merge-patch: desugar PRESERVES absence — undeclared validationState stays
    // absent here (create-defaults land at the upsert, only for a brand-new row).
    expect(p.tracks[0]).toMatchObject({ id: sid, creatorId: 'lnr_default' });
    expect(p.tracks[0]!.validationState).toBeUndefined();
  ...
  it('round-trips tracks and scoped edges through storage', () => {
    ...
    // The EXPORT carries the full shape (create-defaults applied on the new row); the import
    // return is the patch (absence preserved) — the merge-patch split.
