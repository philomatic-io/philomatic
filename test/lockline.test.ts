/**
 * The engine's lock line, mechanically enforced.
 *
 ...
 *      behind it. The chrome.runtime engine host is a shell like the other
...
describe('the lock line applied externally (obsidian plan OB1)', () => {
  it('the obsidian workspace never imports repo src/ — a capability gap is a missing route', () => {
    ...
    if (!existsSync(dir)) return; // the workspace lives in its own repo — nothing to check
    const violations: string[] = [];
