...
describe('Slice 1: concept round-trip', () => {
  ...
  it('desugars a concept: derives id, lexes tags', () => {
    ...
    // Merge-patch: desugar preserves absence — undeclared aliases stay absent
    // rather than defaulting to [] (the default lands at the upsert, on a new row only).
  ...
  it('round-trips a sugared concept through storage (import -> export)', () => {
    ...
    // The export is the FULL canonical shape — create-defaults (aliases: []) applied on the
    // new row; the import return is the patch, so equality is asserted against the full form.
    const exported = engine.exportAll();
    expect(exported.concepts).toEqual([
      {
        id: 'cpt_multiplication',
        name: 'Multiplication',
        description: 'Repeated addition.',
