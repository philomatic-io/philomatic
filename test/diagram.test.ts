/**
 * The diagram drift gate: the committed artifacts must match a regeneration —
...
describe('the edge-taxonomy diagram', () => {
...
describe('the core framework — philomatic-core', () => {
  ...
  it('declares only real edge shapes, and metadata vocabularies only on overlay edges', () => {
    ...
    for (const field of FRAMEWORKS.flatMap((f) => f.metadataFields)) {
      ...
      expect(pairs.every(([s]) => s === 'learner'), `metadata field ${field.name} must ride a learner-overlay edge`).toBe(true);
    }
  });
  ...
  it('selector helpers scope tags and vocabularies correctly', () => {
    ...
    // convention must be machine-readable, not prose, so any front end can port it.
