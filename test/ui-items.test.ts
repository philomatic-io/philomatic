...
const snapshot: Snapshot = {
  version: 1,
  tracks: [{ id: 'syl_1', title: 'Intro to DL', tags: ['#level:intro'], sourceIds: ['src_a'], sourceLevels: [['src_a']], precedes: [], staged: false }],
  sources: [
  ...
  snippets: [{ id: 'snp_1', text: 'chain rule', sourceId: 'src_a', source: 'DL Book', clarifies: ['Backprop'], contradicts: [], tags: ['#key'], raises: [], staged: false }],
};
const questions: QuestionView[] = [
  { id: 'qst_1', text: 'Why converge?', asked: true, answered: false, gap: true, tags: ['#foundational'], about: ['Gradient Descent'], raisedBy: [{ kind: 'snippet', id: 'snp_1', label: 'chain rule', sourceTitle: 'DL Book' }], answeredBy: [], staged: false },
];
...
describe('unified item model', () => {
  ...
  it('counts per kind and aggregates tags + concepts across kinds', () => {
    expect(railCounts(items)).toEqual({ all: 5, backlog: 1, staged: 1, track: 1, concept: 0, source: 2, question: 1, snippet: 1 });
  ...
  it('filters by concept across kinds (OR within): Backprop touches src_b + snp_1', () => {
    ...
    // A TRACK is about the concepts its members are (the topic chips on the
