...
// --squash: ONE-OFF history recut — commit the gated tree as a parentless ORPHAN and force-push,
// collapsing all public history to a single commit (the "first alpha release" reset). Uses the
// SAME allowlist/deny gate as a normal cut, so private docs still cannot leak. Not the routine
// path: normal releases stay stacked/fast-forward (owner ruling 2026-07-21).
const SQUASH = argv.includes('--squash');
...
try {
  ...
  // 4. Write the tree; skip if public already matches (nothing to release) — but a --squash recut
  //    still proceeds even when content is unchanged (it flattens history, not content).
  const tree = git(['write-tree'], { env });
  if (!SQUASH && tree === git(['rev-parse', `${parent}^{tree}`])) {
  ...
  console.log(
    SQUASH
      ? `  mode   : SQUASH — orphan commit, FORCE-PUSH, rewrites all public history (one-off recut)`
      : `  parent : ${parent.slice(0, 9)}  (stacks on top — fast-forward, no rewrite)`,
  );
  ...
  // 5. Commit the gated tree and push. Normal: parented on public HEAD, fast-forward. Squash:
  //    parentless orphan, force-push (the deliberate one-off history rewrite).
  const commit = SQUASH
    ? git(['commit-tree', tree, '-m', msg], { env })
    : git(['commit-tree', tree, '-p', parent, '-m', msg], { env });
  git(['push', ...(SQUASH ? ['--force'] : []), REMOTE, `${commit}:refs/heads/${BRANCH}`]);
  console.log(`✓ ${SQUASH ? 'Recut' : 'Released'} ${commit.slice(0, 9)} → ${REMOTE}/${BRANCH}\n`);
} finally {
