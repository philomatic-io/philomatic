/**
 * Export the store back out as a canonical payload (DATA_MODEL.md). Proves the
...
function buildExport(db: DB): Record<string, unknown> {
  ...
  const payload = {
    version: 2 as const,
    // The framework manifest: what this export ASSUMES — the
    // exporting engine's installed set, stamped fresh per export, never stored. Dormant:
    // imports tolerate and ignore it; absence reads as "core only" (true of older files).
    frameworks: FRAMEWORKS.map((f) => ({ name: f.framework, version: f.version })),
