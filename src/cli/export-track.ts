/**
 * The static export builder: one self-contained HTML file — the
...
export function buildPublicationHtml(bundle: unknown, distDir: string): string {
  ...
  // OG meta: a shared publication link unfurls as a card. Values come from
  // the bundle's own manifest/payload — this page can never say more than the bundle contains.
  const pub = bundle as { publication?: { title?: string; author?: string }; payload?: { tracks?: { goal?: string }[]; sources?: unknown[]; concepts?: unknown[] } };
  const esc = (s: string): string => s.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/"/g, '&quot;');
  const ogTitle = pub.publication?.title;
  const ogBits = [
    ...(pub.payload?.sources?.length ? [`${pub.payload.sources.length} sources`] : []),
    ...(pub.payload?.concepts?.length ? [`${pub.payload.concepts.length} concepts`] : []),
    ...(pub.publication?.author !== undefined ? [`by ${pub.publication.author}`] : []),
  ].join(' · ');
  const ogDesc = pub.payload?.tracks?.[0]?.goal ?? ogBits;
  const og =
    ogTitle !== undefined
