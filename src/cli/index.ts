...
/**
 * Philomatic CLI — a thin adapter over the headless
...
import { loadFrameworkDoc, CaptureError, PhilomaticEngine, ValidationError, rekeyLearner } from '../engine';
  ...
  try {
    switch (command) {
      ...
      case 'import': {
        ...
        const raw = readJson(file);
        // A publication bundle imports as a FORK: lineage recorded, parent
      ...
      case 'export-track': {
        ...
        const fwDoc = loadFrameworkDoc(db ?? ':memory:');
        const bundle = engine.publication(ref, { frameworks: [...(fwDoc.mine !== undefined ? [fwDoc.mine] : []), ...fwDoc.installed] });
      ...
      case 'push': {
        ...
        const bundle = engine.publication(ref, { frameworks: [...(fwDoc.mine !== undefined ? [fwDoc.mine] : []), ...fwDoc.installed] });
        if (!bundle) fail(`"${ref}" is not published — run: philomatic publish "${ref}"`);
        // Deliberate twin: ui/src/client/registry-push.ts pushes with this same fetch shape (the
        // /publish route is the real contract) — keep the two call sites in step by hand.
