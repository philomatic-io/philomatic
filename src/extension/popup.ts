...
/**
 * The capture popup — a capture client of the self-hosted ingest server:
...
// ── Vocabulary from the installed frameworks — never hardcoded in the popup ───────────────────
...
// ── Popup sizing: a footer drag-grip, persisted. Chrome caps
...
// ── Create-a-concept inline: the concept selects gain a
...
// ── The concept palette: the WORKBENCH picker's look, as a vanilla twin
// of ui PickerBox — trigger bar, filter input, icon rows, a dashed create row. The native
// <select> stays in the DOM hidden as the VALUE HOLDER (readForm/drafts keep reading
// sel.value; picking sets it and fires `change`), so this is presentation only. Rebuilt on
// every applyRefData, after values are applied, so the trigger names the current choice.
const CONCEPT_GLYPH =
  '<svg class="glyph" width="12" height="12" viewBox="0 0 256 256" fill="currentColor" aria-hidden="true"><path d="M235.33,116.72,139.28,20.66a16,16,0,0,0-22.56,0l-96,96.06a16,16,0,0,0,0,22.56l96.05,96.06h0a16,16,0,0,0,22.56,0l96.05-96.06a16,16,0,0,0,0-22.56ZM128,224h0L32,128,128,32,224,128Z"/></svg>';

function dressConceptPicker(sel: HTMLSelectElement, placeholder: string): void {
  sel.hidden = true;
  sel.nextElementSibling?.matches('.pm-picker') && sel.nextElementSibling.remove();
  const box = document.createElement('div');
  box.className = 'pm-picker';
  sel.after(box);
  ...
  const setLabel = (): void => {
    const chosen = concepts.find((c) => c.id === sel.value);
    labelSpan.textContent = chosen !== undefined ? trunc(chosen.name, 34) : placeholder;
    labelSpan.className = chosen !== undefined ? 'chosen' : '';
  ...
  search.placeholder = 'type to filter or create…';
  const rows = document.createElement('div');
  rows.className = 'picker-options';
  ...
  const pick = (id: string): void => {
    sel.value = id;
    sel.dispatchEvent(new Event('change', { bubbles: true })); // persist the draft, as before
    close();
  };
  const render = (): void => {
    const q = search.value.trim().toLowerCase();
    rows.replaceChildren();
    for (const c of concepts.filter((c) => !q || c.name.toLowerCase().includes(q))) {
      const row = document.createElement('button');
      row.type = 'button';
      row.className = 'palette-item';
      row.innerHTML = `${CONCEPT_GLYPH} `;
      row.append(document.createTextNode(trunc(c.name, 40)));
      row.addEventListener('click', () => pick(c.id));
      rows.append(row);
    }
    const exact = concepts.some((c) => c.name.toLowerCase() === q);
    if (q.length > 0 && !exact) {
      const create = document.createElement('button');
      create.type = 'button';
      create.className = 'palette-item create';
      create.textContent = `＋ create “${search.value.trim()}”`;
      create.addEventListener('click', () => {
        void createConcept(search.value.trim()).then((id) => {
...
function applyRefData(): void {
  ...
  // The draft may have restored a concept before its option existed — re-apply it now, THEN
  // dress: the palette's trigger names whatever value the select ends up holding.
  if (desiredConcept && concepts.some((c) => c.id === desiredConcept)) el<HTMLSelectElement>('concept').value = desiredConcept;
  dressConceptPicker(el<HTMLSelectElement>('concept'), '— pick a concept —');
  for (const { root, sn } of snipRows) {
    ...
    dressConceptPicker(root.querySelector('.sn-anchor-concept') as HTMLSelectElement, '— no concept —');
...
function readForm(): Draft {
  return {
    ...
    concept: el<HTMLSelectElement>('concept').value,
    snippets,
  };
}
...
function renderSnippets(): void {
  ...
  snippets.forEach((sn, ix) => {
    ...
    dressConceptPicker(anchorConcept, '— no concept —');
...
/** Grow the excerpt box to its text, capped so the popup stays
...
// postage stamp, so the screenshot opens in a FULL TAB (crop.html) at the
...
// ── Modality icon dropdown: the workbench ModalityPicker's
...
async function capture(): Promise<void> {
  ...
  try {
    ...
    // as a weak `resolved` hint so a server adapter (arXiv) can beat it (
