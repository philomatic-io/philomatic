/**
 * DOM-aware selection capture: instead of flattening the selection
...
export function selectionToMarkdown(): string {
  ...
  const walk = (node: Node): string => {
    ...
    if (tag === 'IMG') {
      ...
      // the image tier below would swallow an equation as a picture.
      ...
      // Image capture: every math tier above outranks this — a node
  ...
  const cleaned = out
    ...
    .replace(/[ \t]{2,}/g, ' ')
    .trim()
    // Inline math ALONE on its line is display math: the alt-text
