/**
 * The command layer — application services that COMPOSE core
 ...
 * Future primitives (`remove` = retraction, `update` = supersession) land here.
 */
import { z } from 'zod';
...
import { withPriorSourceFields } from './prior-fields';
...
export function captureSource(ctx: CommandCtx, input: unknown): CaptureSourceResult {
  ...
  // Script-bearing schemes are never a source: `new URL()` happily accepts
  // `javascript:…`, and a stranger's ask submission would then be STORED and handed to every
  // render surface as an href. React blocks these today, but that is the FRAMEWORK's guard,
  // not ours — a server-rendered page or an export would ship live XSS. Reject at the door.
  if (/^(?:javascript|data|vbscript):$/i.test(parsed.protocol)) {
    throw new CaptureError(`unsupported URL scheme: ${parsed.protocol}`);
  }
  ...
  // Clobber policy: adapter output fills STILL-EMPTY
  ...
  // Finish the clobber policy this function already declares. It protected title,
  // author and duration and left personalUrl, bibliographicUrl, status and TAGS to be nulled by
  // a payload that never mentioned them — reachable by a stranger, since an ask submission for a
  // URL the owner already holds lands on the owner's own row.
  ctx.importPayload(withPriorSourceFields(store, payload));

  // Re-capture revives, explicitly: capturing a URL whose source is removed
...
export function captureSnippet(ctx: CommandCtx, input: unknown): CaptureSnippetResult {
  ...
  // Re-capture revives, explicitly: capturing a removed passage — or a passage
...
// ── Edit primitives: remove = retraction, restore, update = supersession ──
// Generic by construction: one primitive per operation, dispatched on the ref's id prefix — the
// command surface does not grow per entity kind.
...
/** Restore = the counter-observation. Minimal-ancestors: restoring a snippet
...
// What update() may touch, per kind. Identity-participating fields are named
...
const IDENTITY_FIELDS: Record<EditKind, ReadonlyArray<string>> = {
  ...
  question: ['text'],
  // `name` slugs the concept id, but update() handles it as RENAME-BY-SUPERSESSION (the
  // track pattern) — mint the new id, re-point edges, retract the old.
  concept: [],
...
const EDITABLE: Record<EditKind, z.ZodType<Record<string, unknown>>> = {
  ...
  concept: z.object({
    name: z.string().min(1).optional(), // rename-by-supersession; handled in update()
...
/**
 * Update = honest whole-value supersession: read–modify–write done ONCE, in the engine
 ...
 * True per-field supersession arrives with the assertion layer.
 */
// ── The publish act ──────────────────────────────────────
...
export function publishTrack(ctx: CommandCtx, input: unknown): EditResult {
  ...
  const alias = req.as !== undefined ? { as: { trackId: trackId(req.as), title: req.as } } : {};
  if (cur.published) {
    // Already published: the only restamp allowed is ADOPTING an alias — how a fork,
    // refused upstream, republishes as its own version. Same license, same first-published date.
    if (req.as !== undefined && cur.published.as?.title !== req.as) {
      ctx.importPayload({ version: 2, tracks: [{ ...cur, published: { ...cur.published, ...alias } }] });
      return { version: CAPTURE_VERSION, kind: 'track', targetId: id, changed: true };
    }
    return { version: CAPTURE_VERSION, kind: 'track', targetId: id, changed: false };
  }
  ctx.importPayload({
    version: 2,
    tracks: [{ ...cur, published: { at: ctx.now(), license: req.license ?? DEFAULT_LICENSE, ...alias } }],
  });
  return { version: CAPTURE_VERSION, kind: 'track', targetId: id, changed: true };
}

/** Clear the stamp: distribution stops (routes 404, future aggregates exclude it). The
 *  command's surface must stay honest that copies made while public persist. */
...
export function update(ctx: CommandCtx, input: unknown): EditResult {
  ...
  switch (kind) {
    ...
    case 'concept': {
      ...
      // Rename-by-supersession: the name slugs the id, so a name
      // change cannot mutate in place. Mint the concept under its NEW id (all other fields
      // carried), re-point every edge that referenced the old id, and retract the old —
      // restorable, identity stays content-derived. Concepts are the cleanest case for this:
      // every reference is an edge by id (ABOUT/INCLUDES/PREREQUISITE_OF/LINK/CLARIFIES/…) and
      // `source.about` is edge-derived, so nothing dangles. No trackContextId to carry (a
      // concept is never a PRECEDES context).
      const newName = typeof fields.name === 'string' ? fields.name.trim() : undefined;
      // A rename that slugs to the SAME id (case/spacing only) is an in-place update below,
      // not a supersession — the collision check would refuse the concept against itself.
      if (newName !== undefined && newName !== cur.name && conceptId(newName) !== id) {
        const newId = conceptId(newName);
        const priorAtNewId = store.concepts.find((c) => c.id === newId);
        if (priorAtNewId && !isRetracted(store, newId)) {
          throw new CaptureError(`a concept named “${newName}” already exists — merging two concepts is a Phase-2 dedup action`);
        }
        const movedEdges = store.edges
          .filter((e) => e.srcId === id || e.dstId === id)
          .map((e) => ({ ...e, srcId: e.srcId === id ? newId : e.srcId, dstId: e.dstId === id ? newId : e.dstId }));
        ctx.importPayload({ version: 2, concepts: [{ ...next, id: newId, name: newName }], edges: movedEdges });
        appendEditEvents(ctx, learnerId, 'RETRACTED', [{ targetType: 'concept', targetId: id }], ctx.now());
        // Round-trip revive — renaming BACK to an earlier name must un-retract that id.
        if (priorAtNewId && isRetracted(store, newId)) {
          appendEditEvents(ctx, learnerId, 'RESTORED', [{ targetType: 'concept', targetId: newId }], ctx.now());
        }
    ...
    case 'snippet': {
      ...
      // (already edge-carried, no new mechanism).
      ...
      // Edit-by-supersession (the raw-source toggle made text edits
      ...
      // would refuse the snippet against itself.
      ...
      if (textChanged && newId !== id) {
        ...
        // Round-trip revive: editing BACK to earlier text lands on a
...
// STAGED is the PENDING-VALIDATION state for any entity — an adapter/LLM proposal or something
// you parked yourself. It leaves that state three ways, and an un- verb may only ever REVERSE
// its verb, so the two VERDICTS get their own words:
//   accept  → the proposal becomes an ordinary entity (marker off) + an ACCEPTED verdict event
//   reject  → the proposal is retracted (append-only)      + a REJECTED verdict event
//   unstage → the marker simply comes off, NO verdict      + an UNSTAGED event (≈ UNCONSUMED)
// Every verdict is logged, so the disposition log answers "why is this in my graph" and doubles
// as labeled data. Rejections are signal — never physical deletions.

/** Resolve any entity reference (id or natural key) for the staging verbs. */
export function resolveStageRef(store: CanonicalPayload, ref: string): { kind: EditKind; id: string } {
  return resolveEditRef(store, ref);
}

/** Park/propose: write the STAGED fact edge + its log event. Idempotent (one fact edge). */
export function stageEntity(ctx: CommandCtx, ref: string, opts: VerbOptions = {}): { kind: EditKind; targetId: string } {
  const { kind, id } = resolveEditRef(ctx.exportAll(), ref);
  recordVerb(ctx, 'STAGED', kind, id, opts);
