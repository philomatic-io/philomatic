/**
 * The capture contract — the engine's own,
...
/**
 * Durable facts a write-time adapter may contribute. Folded
...
export const CaptureSourceInput = z.object({
  ...
  /** Whose capture this is (STAGED/RESTORED land under this learner) — per-learner tenancy;
...
export interface CaptureSourceResult {
   ...
   *  back", recorded as an explicit RESTORED event. Additive field. */
  revived?: boolean;
...
// ── The edit contract: remove = retraction, restore, update = supersession ────
...
/** Publish a track: the explicit act that stamps a
...
export const PublishInput = z.object({
  version: z.literal(CAPTURE_VERSION).optional(),
  ref: z.string().min(1),
  /** Named at the act. Omitted → DEFAULT_LICENSE. */
  license: z.string().min(1).optional(),
  /** Publish AS this title: the bundle carries the derived trackId+this title instead of
   *  the local identity — a fork publishing as its own version under its own name. */
  as: z.string().min(1).optional(),
});
export type PublishInput = z.infer<typeof PublishInput>;
export const UnpublishInput = EditRefInput;

/** The default license a publish stamps when none is named. */
export const DEFAULT_LICENSE = 'CC-BY-SA-4.0';

/** A publication bundle arriving for import (= a FORK). The manifest is
...
export const PublicationBundleInput = z.object({
  ...
  publication: z.object({
    ...
    authorKey: z.string().optional(), // author signing — absent on unsigned bundles (unattested forever)
    signature: z.string().optional(),
  }),
  payload: z.record(z.unknown()),
  /** Carried framework definitions — validated at INSTALL time against the payload's
   *  hash-covered manifest; the bundle parse only preserves them (a strict parse was silently
   *  stripping the field, so the registry stored def-less bundles). */
  frameworkDefs: z.array(z.unknown()).optional(),
});
export type PublicationBundleInput = z.infer<typeof PublicationBundleInput>;

/**
 * Un-assert a structural edge by its full coordinates (the concept
 ...
 * true retraction when the assertion layer gives edges ids. Overlay verbs are
...
/**
 * Assert a structural edge (the inverse of unlink). The UI
 ...
 *  identity-participating fields are rejected with the reason. */
