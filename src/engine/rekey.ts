/**
 * Learner re-key — the pure map that moves a learner's
 ...
 * payload mentions learners — content ids never include them (the keep-it-pure rule).
 */
...
export function rekeyLearner(payload: CanonicalPayload, oldId: string, newId: string): CanonicalPayload {
  ...
  if (payload.learners.some((l) => l.id === newId)) {
    // Merging two existing learners would conflate their behavior — never silently.
