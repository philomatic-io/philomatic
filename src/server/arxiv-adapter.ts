/**
 * The arXiv source adapter — the first real adapter (the enrichment runtime was gated on one landing to
...
export function arxivAdapter(fetcher: typeof fetch = fetch): SourceAdapter {
  return {
    ...
    // The LaTeXML build carries MathML with x-tex annotations — the acquisition tier the
    // html math resolvers were made for. Older papers (pre-~2023) 404 there; since PDF text
    // extraction landed, the PDF outranks the abs page — full text beats the
    // abstract — and abs stays as the graceful floor for image-only scans.
    acquireUrls: (url) => {
      const id = arxivId(url);
      return id
        ? [`https://arxiv.org/html/${id}`, `https://arxiv.org/pdf/${id}`, `https://arxiv.org/abs/${id}`]
        : [];
    },
