/**
 * Edge endpoint taxonomy (DATA_MODEL.md).
 *
 ...
 * The v1 taxonomy collapses into this one via io/migrate.
 */
...
export const ENDPOINT_RULES: Record<EdgeType, ReadonlyArray<Pair>> = {
  ...
  // The pending-validation state: ANY entity can be staged —
  // adapter-proposed or self-backlogged. CONSUMED stays source-only read state.
  STAGED: [
    ['learner', 'source'],
    ['learner', 'concept'],
    ['learner', 'question'],
    ['learner', 'snippet'],
    // Tracks joined for the survey→track pass: a whole PROPOSED track is
    // stageable, and accepting it is the explicit INCLUDES gesture at track granularity.
    ['learner', 'track'],
  ],
