/**
 * The one schema DDL, shared by both drivers of the dialect seam:
 * `db.ts` (better-sqlite3, node) and `db-browser.ts` (sql.js WASM, browser).
 * Applied programmatically (CREATE TABLE IF NOT EXISTS) to keep tests hermetic on in-memory DBs;
 * drizzle-kit migration files arrive once the model stops churning.
 */
export const DDL = `
