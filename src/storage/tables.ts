/**
 * Drizzle table definitions. Per-entity node tables plus one
...
// PHYSICAL NAME FROZEN: the table stays `syllabi` on disk — like
...
export const tracks = sqliteTable('syllabi', {
  ...
  origin: text('origin'), // JSON {trackId, publishedAt, contentHash, url?} — fork lineage
  createdAt: integer('created_at'),
  updatedAt: integer('updated_at'),
});
 ...
 * edges) lets a track-scoped ordering (PRECEDES) coexist per track.
 */
