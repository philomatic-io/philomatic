...
export function buildItems(
  ...
  concepts: { id: string; name: string; tracked: boolean; tags?: string[] }[] = [],
  ...
  // The concepts a track is ABOUT — for the list card's chips (owner, 2026-07-23). The union of
  ...
  /** A track's one-line meta — the SAME shape for every track, no anchor-mode branch (owner,
   *  2026-07-23). Its member sources always count; when it also includes concepts, they lead.
  ...
  const items: Item[] = [
    ...
    ...questions.map((q): Item => {
      // "raised by <source>" (owner, 2026-07-24) — the source that raised it: a source
