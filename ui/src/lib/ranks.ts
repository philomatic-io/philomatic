/**
 * Concept ranks, derived from the framework-declared hierarchy:
...
/** Is this rendered tag part of a declared hierarchy? — clients sort/nest from THIS, never
 *  from tag-name literals (declarations, never names). */
export const isHierarchyTag = (t: string): boolean => PARENT_TAGS.has(t) || ATTACH_TAGS.has(t);
 ...
 *  render taxonomy from THIS, never from tag-name literals. */
