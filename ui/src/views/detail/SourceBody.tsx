  ...
  return (
    <>
      {/* The identity URL only — its own canonical link, shown once. The personal link lives in
          its labeled fact row below (showing it here too was a duplicate). The
...
/** A source's passages, rendered as the LAST group of its Connections list.
 *  They are a connection — SNIPPET_OF — and the only one whose payload is worth reading in
 *  place, which is why they render as quoted boxes rather than rows. Filed under Connections'
 *  own kind-tinted "Snippets" heading, they stop being a section that looked like a second
 *  index of the source. */
 ...
 *  podcasts-on-web — so it's correctable here). */
export function SourceFacts({
  ...
  // Pencil-toggled like the title/goal editors: read-only text
  ...
  // here in its labeled row (it used to also duplicate at the top).
      ...
      <div className="fact-row">
        ...
        {editingAuthor ? (
          ...
          <span className="fact-value" title={source.author ?? ''}>
            <span className={editMode ? 'fact-value-text editable-text' : 'fact-value-text'} onClick={() => editMode && setEditingAuthor(true)}>{source.author ?? <span className="fact-empty">add authors…</span>}</span>
        ...
        <div className="fact-row">
          ...
          {editingLink ? (
            ...
            <span className="fact-value" title={source.personalUrl ?? ''}>
              <span className={editMode ? 'fact-value-text editable-text' : 'fact-value-text'} onClick={(e) => { if (editMode && !source.personalUrl) { e.preventDefault(); setEditingLink(true); } }}>
                {source.personalUrl ? (
