...
export function TrackBody({
  ...
  /** Per-source open-question / snippet counts — shown on each source row (owner, 2026-07-24). */
  ...
  const includeConcepts = async (names: string[]) => {
    try {
      // Resolve/create each, link each, and undo the whole batch as one — so adding several
      // concepts at once is one Ctrl+Z, un-minting any the gesture created.
      ...
      pushUndo(`include ${made.length === 1 ? `“${made[0]!.name.slice(0, 30)}”` : `${made.length} concepts`}`, async () => {
        for (const c of made) {
      ...
      await refresh();
      notify(made.length === 1 ? `Included “${made[0]!.name}” ✓` : `Included ${made.length} concepts ✓`);
    } catch (e) {
  ...
  return (
    <>
      ...
      {/* ONE track view (owner design 2026-07-23) — no by-sources/by-concept toggle and no
          branch on how the track is anchored. The included concepts are NOT repeated as chips
          up here (owner, 2026-07-23): they are the numbered headings in the Concepts section,
      ...
      <TrackSection
        ...
        onRemoveConcept={(id, name) => void unIncludeConcept(id, name)}
